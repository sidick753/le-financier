...
export function NotifBell({ count = 0 }: { count?: number }) {
  return (
    ...
    >
      ...
      {count > 0 && (
        <span className="absolute -right-1.5 -top-1.5 flex min-w-[18px] items-center justify-center rounded-full border-2 border-white bg-red-600 px-1 py-px text-[9px] font-bold text-white leading-none">
          {count}
        </span>
      )}
    </button>
