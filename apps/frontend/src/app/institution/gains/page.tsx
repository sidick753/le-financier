...
export default function InstitutionGainsPage() {
  const {
    upcoming,
    payments,
    isLoading,
  ...
  return (
    <>
      <header className="sticky top-0 z-10 flex h-15 items-center justify-between gap-4 border-b border-slate-200 bg-white/90 px-8 backdrop-blur-md">
        <div>
          <p className="text-[18px] font-bold tracking-tight text-slate-900">Gains & Revenus</p>
          <p className="text-xs text-slate-500">
            Historique des revenus de l&apos;institution et prochaines échéances
          </p>
        </div>
      ...
      <div className="p-8 pb-16">
            ...
            <div className="space-y-3">
              {upcoming.slice(0, 4).map((item) => (
                <div key={item.id} className="flex items-start gap-3">
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-brand-50 text-xs">
                    📅
                  </div>
        ...
        <div className="mt-4 rounded-xl border border-gray-200 bg-white p-5">
          ...
          <p className="mt-1 text-xs text-gray-400">
            Les revenus de crowdlending sont soumis à l&apos;IS. Consultez votre conseiller fiscal.
          </p>
          <button
