...
function FavoriteCard({ entry, onRemove }: { entry: WatchlistEntry; onRemove: (fundingRequestId: string) => void }) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_2px_8px_rgba(15,23,42,0.04)] transition hover:shadow-[0_4px_20px_rgba(15,23,42,0.08)]">
      ...
      <p className="mb-4 text-[13px] font-medium text-brand-700">{opp.title}</p>
      ...
      {/* CTA */}
      <Link
        href={`/institution/deal-flow/${opp.id}`}
        className="flex items-center justify-center gap-2 rounded-[10px] bg-brand-700 py-2.5 text-[13px] font-semibold text-white transition hover:bg-brand-800"
      >
        Voir le détail
...
export default function InstitutionFavorisPage() {
          ...
          {!isLoading && entries.length === 0 && (
            <div className="col-span-3 flex flex-col items-center rounded-2xl border border-dashed border-slate-200 bg-white px-8 py-16 text-center">
              ...
              <p className="mt-1 text-[13px] text-slate-500">
                Parcourez le deal flow et cliquez sur « Ajouter à mes favoris » pour les retrouver ici.
              </p>
              <Link
                href="/institution/deal-flow"
                className="mt-4 rounded-[10px] bg-brand-700 px-4 py-2 text-[13px] font-semibold text-white transition hover:bg-brand-800"
              >
                Explorer le deal flow
              </Link>
            </div>
          )}
